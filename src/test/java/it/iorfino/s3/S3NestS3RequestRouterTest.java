...
public class S3NestS3RequestRouterTest {
    ...
    /**
     * Verifies that a parsed S3 request is routed to the handler associated
     * with its operation.
     */
    @Test
    void shouldRouteRequestToMatchingHandler() {
        ...
        S3NestS3OperationHandler handler = request -> {
        ...
        S3NestS3RequestRouter router = new S3NestS3RequestRouter(
                Map.of(S3Operation.GET_OBJECT, handler)
        );
    ...
    /**
     * Verifies that routing fails deterministically when no handler is
     * registered for the requested S3 operation.
     */
    @Test
    void shouldRejectRequestWithoutMatchingHandler() {
        ...
        S3NestS3RequestRoutingException exception = assertThrows(
                S3NestS3RequestRoutingException.class,
                () -> router.route(request)
        );

        assertEquals(
                "Unsupported S3 operation: GET_OBJECT",
                exception.getMessage()
        );
    }

    /**
     * Verifies that the router passes the original request instance to the
     * operation handler without modifying it.
     */
    @Test
    void shouldPassOriginalRequestToHandler() {
        ...
        S3NestS3RequestRouter router = new S3NestS3RequestRouter(
                Map.of(S3Operation.PUT_OBJECT, handler)
        );
    ...
    /**
     * Verifies that different S3 operations are routed to their respective
     * handlers.
     */
    @Test
    void shouldRouteDifferentOperationsToTheirHandlers() {
        ...
        S3NestS3OperationHandler getHandler = request -> {
            handledOperation.set(S3Operation.GET_OBJECT);
            return new S3NestS3EmptyResult();
        };

        S3NestS3OperationHandler deleteHandler = request -> {
        ...
        S3NestS3Request getRequest = new S3NestS3Request(
                S3Operation.GET_OBJECT,
                "my-bucket",
                "my-object",
                Map.of(),
        ...
        S3NestS3Request deleteRequest = new S3NestS3Request(
    ...
    /**
     * Verifies that the router returns the result produced by the operation
     * handler.
     */
    @Test
    void shouldReturnHandlerResult() {
        S3NestS3OperationResult expectedResult =
                new S3NestS3EmptyResult();
