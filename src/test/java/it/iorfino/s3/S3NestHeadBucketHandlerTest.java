...
public class S3NestHeadBucketHandlerTest {
  ...
  /**
   * Verifies that the HEAD_BUCKET handler returns an empty result when the requested bucket exists.
   */
  @Test
  void shouldHandleExistingBucket() {
    AtomicReference<String> checkedBucket = new AtomicReference<>();

    S3NestS3BucketStorage storage =
        new S3NestS3BucketStorage() {
          @Override
    ...
    S3NestHeadBucketHandler handler = new S3NestHeadBucketHandler(storage);
  ...
  /**
   * Verifies that the HEAD_BUCKET handler rejects a request when the requested bucket does not
   * exist.
   */
  @Test
  void shouldRejectMissingBucket() {
    ...
    assertThrows(S3NestS3BucketNotFoundException.class, () -> handler.handle(request));
  }
}
