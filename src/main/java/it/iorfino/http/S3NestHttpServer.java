 ...
 *
 * <p>The server owns its listening socket and request processing resources. Implementations must
 * not rely on global mutable server state.
 *
 * <p>A configured port of {@code 0} requests automatic ephemeral port allocation. The actual
 * assigned port can be retrieved through {@link #port()} after the server has started.
 */
public interface S3NestHttpServer {
   ...
   *
   * <p>Calling this method repeatedly must have deterministic behaviour.
   */
  void stop();
