 ...
 *
 * <p>This implementation uses the Java 21 HTTP server and virtual threads to provide the HTTP
 * infrastructure required by the S3 protocol layer.
 *
 * <p>The implementation is responsible for TCP port binding, HTTP request adaptation, request
 * execution and resource lifecycle management. S3 protocol semantics are delegated to the
 * configured {@link S3NestHttpHandler}.
 *
 * <p>Each instance owns its own listening socket and request processing executor. No global server
 * state is used.
 */
public final class S3NestHttpServerImpl implements S3NestHttpServer {
   ...
   *
   * <p>A port of {@code 0} requests automatic ephemeral port allocation from the operating system.
   *
   ...
   *
   * <p>The server uses a virtual thread per request, allowing multiple requests to be processed
   * concurrently without serializing request handling through a single worker thread.
   *
    ...
    try {
      ...
      this.server.createContext(
          ...
          exchange -> {
            ...
            response
                .headers()
                .forEach((name, values) -> exchange.getResponseHeaders().put(name, values));
   ...
   *
   * <p>The listening socket is closed and the request processing executor is shut down. Calling
   * this method when the server is already stopped has no effect.
   */
  @Override
  public void stop() {
    if (server == null) {
   ...
   *
   * <p>This method also returns the actual operating-system-assigned port when the server was
   * configured with port {@code 0}.
   *
