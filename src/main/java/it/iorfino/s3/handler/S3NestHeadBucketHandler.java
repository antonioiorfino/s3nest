 ...
 *
 * <p>The handler checks whether the requested bucket exists through the
 * bucket storage port.</p>
 *
 * <p>A successful existence check produces an empty S3 operation result.
 * The handler does not depend on HTTP transport details or on a concrete
 * storage implementation.</p>
 */
public final class S3NestHeadBucketHandler
    ...
    public S3NestS3OperationResult handle(S3NestS3Request request) {
        ...
        if (!bucketExists) {
            throw new S3NestS3BucketNotFoundException(
                    request.bucket()
            );
        }
