 ...
 *
 * <p>The router operates exclusively on {@link S3NestS3Request} and {@link S3Operation}. It does
 * not inspect HTTP methods or paths.
 */
public final class S3NestS3RequestRouter {
   ...
   */
  public S3NestS3RequestRouter(Map<S3Operation, S3NestS3OperationHandler> handlers) {
    this.handlers = Map.copyOf(Objects.requireNonNull(handlers, "handlers"));
  }

  /**
   * Routes the supplied S3 request to the handler associated with its operation.
   *
   * <p>The result produced by the handler is returned to the caller so that the response layer can
   * translate the operation result into an HTTP response.
   *
   * @param request the parsed S3 request
   * @return the result produced by the operation handler
   * @throws S3NestS3RequestRoutingException if no handler is registered for the requested operation
   */
  public S3NestS3OperationResult route(S3NestS3Request request) {
    ...
    if (handler == null) {
      throw new S3NestS3RequestRoutingException("Unsupported S3 operation: " + request.operation());
    }
